	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			else
			{
				if (Log::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
				{
					DM_CORE_ERROR("Instrumentor could not open results file '{0}'.", filepath);
				}
			}
		}
...
#define DM_PROFILE_BEGIN_SESSION(name, filepath) ::Dimon::Instrumentor::Get().BeginSession(name, filepath)
#define DM_PROFILE_END_SESSION() ::Dimon::Instrumentor::Get().EndSession()
#define DM_PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = ::Dimon::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
											   ::Dimon::InstrumentationTimer timer##line(fixedName##line.Data)
